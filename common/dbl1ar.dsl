...
<!ENTITY % ar.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//AR"
         "dbl1ar.ent">
%ar.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-ar">
<style-specification-body>

;; $Id: dbl1ar.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (ar-author-string #!optional (author (current-node)))
...
(define (ar-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-ar-start-quote%
						       "%t"
						       %gentext-ar-end-quote%))
...
(define (gentext-ar-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (ar-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (ar-auto-xref-indirect-connector before) 
...
;;
(define %generate-ar-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (ar-element-name)
  (list
...
(define (gentext-ar-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (ar-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-ar-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-ar-element-name-space giname)
...
(define (local-ar-intra-label-sep)
  (list))

(define (ar-intra-label-sep)
  (list
...
(define (gentext-ar-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-ar-intra-label-sep)))
	 (sep    (assoc name (ar-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-ar-label-title-sep)
  (list))

(define (ar-label-title-sep)
  (list
...
(define (gentext-ar-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-ar-label-title-sep)))
	 (sep    (assoc name (ar-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (ar-label-number-format-list)
  (list
...
(define (ar-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (ar-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-ar$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (ar-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-ar-start-quote% (dingbat "ldquo"))

(define %gentext-ar-end-quote% (dingbat "rdquo"))

(define %gentext-ar-start-nested-quote% (dingbat "lsquo"))

(define %gentext-ar-end-nested-quote% (dingbat "rsquo"))

(define %gentext-ar-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-ar-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-ar-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-ar-page% "")

(define %gentext-ar-and% "&and;")

(define %gentext-ar-listcomma% "&listcomma;")

(define %gentext-ar-lastlistcomma% "&lastlistcomma;")

(define %gentext-ar-bibl-pages% "&Pgs;")

(define %gentext-ar-endnotes% "&Notes;")

(define %gentext-ar-table-endnotes% "&TableNotes;:")

(define %gentext-ar-index-see% "&See;")

(define %gentext-ar-index-seealso% "&SeeAlso;")


(define (gentext-ar-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-ar-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-ar-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-ar-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-ar-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-ar-nav-home home)
