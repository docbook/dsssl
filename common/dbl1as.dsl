...
<!ENTITY % as.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//AS"
         "dbl1as.ent">
%as.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-as">
<style-specification-body>

;; $Id: dbl1as.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (as-author-string #!optional (author (current-node)))
...
(define (as-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-as-start-quote%
						       "%t"
						       %gentext-as-end-quote%))
...
(define (gentext-as-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (as-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (as-auto-xref-indirect-connector before) 
...
;;
(define %generate-as-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (as-element-name)
  (list
...
(define (gentext-as-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (as-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-as-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-as-element-name-space giname)
...
(define (local-as-intra-label-sep)
  (list))

(define (as-intra-label-sep)
  (list
...
(define (gentext-as-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-as-intra-label-sep)))
	 (sep    (assoc name (as-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-as-label-title-sep)
  (list))

(define (as-label-title-sep)
  (list
...
(define (gentext-as-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-as-label-title-sep)))
	 (sep    (assoc name (as-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (as-label-number-format-list)
  (list
...
(define (as-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (as-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-as$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (as-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-as-start-quote% (dingbat "ldquo"))

(define %gentext-as-end-quote% (dingbat "rdquo"))

(define %gentext-as-start-nested-quote% (dingbat "lsquo"))

(define %gentext-as-end-nested-quote% (dingbat "rsquo"))

(define %gentext-as-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-as-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-as-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-as-page% "")

(define %gentext-as-and% "&and;")

(define %gentext-as-listcomma% "&listcomma;")

(define %gentext-as-lastlistcomma% "&lastlistcomma;")

(define %gentext-as-bibl-pages% "&Pgs;")

(define %gentext-as-endnotes% "&Notes;")

(define %gentext-as-table-endnotes% "&TableNotes;:")

(define %gentext-as-index-see% "&See;")

(define %gentext-as-index-seealso% "&SeeAlso;")


(define (gentext-as-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-as-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-as-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-as-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-as-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-as-nav-home home)
