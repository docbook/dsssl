...
<!ENTITY % az.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//AZ"
         "dbl1az.ent">
%az.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-az">
<style-specification-body>

;; $Id: dbl1az.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (az-author-string #!optional (author (current-node)))
...
(define (az-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-az-start-quote%
						       "%t"
						       %gentext-az-end-quote%))
...
(define (gentext-az-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (az-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (az-auto-xref-indirect-connector before) 
...
;;
(define %generate-az-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (az-element-name)
  (list
...
(define (gentext-az-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (az-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-az-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-az-element-name-space giname)
...
(define (local-az-intra-label-sep)
  (list))

(define (az-intra-label-sep)
  (list
...
(define (gentext-az-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-az-intra-label-sep)))
	 (sep    (assoc name (az-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-az-label-title-sep)
  (list))

(define (az-label-title-sep)
  (list
...
(define (gentext-az-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-az-label-title-sep)))
	 (sep    (assoc name (az-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (az-label-number-format-list)
  (list
...
(define (az-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (az-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-az$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (az-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-az-start-quote% (dingbat "ldquo"))

(define %gentext-az-end-quote% (dingbat "rdquo"))

(define %gentext-az-start-nested-quote% (dingbat "lsquo"))

(define %gentext-az-end-nested-quote% (dingbat "rsquo"))

(define %gentext-az-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-az-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-az-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-az-page% "")

(define %gentext-az-and% "&and;")

(define %gentext-az-listcomma% "&listcomma;")

(define %gentext-az-lastlistcomma% "&lastlistcomma;")

(define %gentext-az-bibl-pages% "&Pgs;")

(define %gentext-az-endnotes% "&Notes;")

(define %gentext-az-table-endnotes% "&TableNotes;:")

(define %gentext-az-index-see% "&See;")

(define %gentext-az-index-seealso% "&SeeAlso;")


(define (gentext-az-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-az-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-az-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-az-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-az-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-az-nav-home home)
