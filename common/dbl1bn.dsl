...
<!ENTITY % bn.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//BN"
         "dbl1bn.ent">
%bn.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-bn">
<style-specification-body>

;; $Id: dbl1bn.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (bn-author-string #!optional (author (current-node)))
...
(define (bn-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-bn-start-quote%
						       "%t"
						       %gentext-bn-end-quote%))
...
(define (gentext-bn-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (bn-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (bn-auto-xref-indirect-connector before) 
...
;;
(define %generate-bn-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (bn-element-name)
  (list
...
(define (gentext-bn-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (bn-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-bn-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-bn-element-name-space giname)
...
(define (local-bn-intra-label-sep)
  (list))

(define (bn-intra-label-sep)
  (list
...
(define (gentext-bn-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-bn-intra-label-sep)))
	 (sep    (assoc name (bn-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-bn-label-title-sep)
  (list))

(define (bn-label-title-sep)
  (list
...
(define (gentext-bn-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-bn-label-title-sep)))
	 (sep    (assoc name (bn-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (bn-label-number-format-list)
  (list
...
(define (bn-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (bn-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-bn$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (bn-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-bn-start-quote% (dingbat "ldquo"))

(define %gentext-bn-end-quote% (dingbat "rdquo"))

(define %gentext-bn-start-nested-quote% (dingbat "lsquo"))

(define %gentext-bn-end-nested-quote% (dingbat "rsquo"))

(define %gentext-bn-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-bn-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-bn-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-bn-page% "")

(define %gentext-bn-and% "&and;")

(define %gentext-bn-listcomma% "&listcomma;")

(define %gentext-bn-lastlistcomma% "&lastlistcomma;")

(define %gentext-bn-bibl-pages% "&Pgs;")

(define %gentext-bn-endnotes% "&Notes;")

(define %gentext-bn-table-endnotes% "&TableNotes;:")

(define %gentext-bn-index-see% "&See;")

(define %gentext-bn-index-seealso% "&SeeAlso;")


(define (gentext-bn-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-bn-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-bn-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-bn-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-bn-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-bn-nav-home home)
