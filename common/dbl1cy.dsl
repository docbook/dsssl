...
<!ENTITY % cy.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//CY"
         "dbl1cy.ent">
%cy.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-cy">
<style-specification-body>

;; $Id: dbl1cy.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (cy-author-string #!optional (author (current-node)))
...
(define (cy-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-cy-start-quote%
						       "%t"
						       %gentext-cy-end-quote%))
...
(define (gentext-cy-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (cy-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (cy-auto-xref-indirect-connector before) 
...
;;
(define %generate-cy-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (cy-element-name)
  (list
...
(define (gentext-cy-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (cy-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-cy-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-cy-element-name-space giname)
...
(define (local-cy-intra-label-sep)
  (list))

(define (cy-intra-label-sep)
  (list
...
(define (gentext-cy-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-cy-intra-label-sep)))
	 (sep    (assoc name (cy-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-cy-label-title-sep)
  (list))

(define (cy-label-title-sep)
  (list
...
(define (gentext-cy-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-cy-label-title-sep)))
	 (sep    (assoc name (cy-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (cy-label-number-format-list)
  (list
...
(define (cy-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (cy-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-cy$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (cy-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-cy-start-quote% (dingbat "ldquo"))

(define %gentext-cy-end-quote% (dingbat "rdquo"))

(define %gentext-cy-start-nested-quote% (dingbat "lsquo"))

(define %gentext-cy-end-nested-quote% (dingbat "rsquo"))

(define %gentext-cy-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-cy-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-cy-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-cy-page% "")

(define %gentext-cy-and% "&and;")

(define %gentext-cy-listcomma% "&listcomma;")

(define %gentext-cy-lastlistcomma% "&lastlistcomma;")

(define %gentext-cy-bibl-pages% "&Pgs;")

(define %gentext-cy-endnotes% "&Notes;")

(define %gentext-cy-table-endnotes% "&TableNotes;:")

(define %gentext-cy-index-see% "&See;")

(define %gentext-cy-index-seealso% "&SeeAlso;")


(define (gentext-cy-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-cy-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-cy-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-cy-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-cy-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-cy-nav-home home)
