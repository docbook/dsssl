...
<!ENTITY % ga.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//GA"
         "dbl1ga.ent">
%ga.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-ga">
<style-specification-body>

;; $Id: dbl1ga.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (ga-author-string #!optional (author (current-node)))
...
(define (ga-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-ga-start-quote%
						       "%t"
						       %gentext-ga-end-quote%))
...
(define (gentext-ga-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (ga-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (ga-auto-xref-indirect-connector before) 
...
;;
(define %generate-ga-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (ga-element-name)
  (list
...
(define (gentext-ga-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (ga-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-ga-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-ga-element-name-space giname)
...
(define (local-ga-intra-label-sep)
  (list))

(define (ga-intra-label-sep)
  (list
...
(define (gentext-ga-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-ga-intra-label-sep)))
	 (sep    (assoc name (ga-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-ga-label-title-sep)
  (list))

(define (ga-label-title-sep)
  (list
...
(define (gentext-ga-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-ga-label-title-sep)))
	 (sep    (assoc name (ga-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (ga-label-number-format-list)
  (list
...
(define (ga-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (ga-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-ga$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (ga-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-ga-start-quote% (dingbat "ldquo"))

(define %gentext-ga-end-quote% (dingbat "rdquo"))

(define %gentext-ga-start-nested-quote% (dingbat "lsquo"))

(define %gentext-ga-end-nested-quote% (dingbat "rsquo"))

(define %gentext-ga-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-ga-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-ga-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-ga-page% "")

(define %gentext-ga-and% "&and;")

(define %gentext-ga-listcomma% "&listcomma;")

(define %gentext-ga-lastlistcomma% "&lastlistcomma;")

(define %gentext-ga-bibl-pages% "&Pgs;")

(define %gentext-ga-endnotes% "&Notes;")

(define %gentext-ga-table-endnotes% "&TableNotes;:")

(define %gentext-ga-index-see% "&See;")

(define %gentext-ga-index-seealso% "&SeeAlso;")


(define (gentext-ga-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-ga-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-ga-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-ga-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-ga-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-ga-nav-home home)
