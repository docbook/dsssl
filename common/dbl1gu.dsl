...
<!ENTITY % gu.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//GU"
         "dbl1gu.ent">
%gu.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-gu">
<style-specification-body>

;; $Id: dbl1gu.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (gu-author-string #!optional (author (current-node)))
...
(define (gu-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-gu-start-quote%
						       "%t"
						       %gentext-gu-end-quote%))
...
(define (gentext-gu-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (gu-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (gu-auto-xref-indirect-connector before) 
...
;;
(define %generate-gu-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (gu-element-name)
  (list
...
(define (gentext-gu-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (gu-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-gu-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-gu-element-name-space giname)
...
(define (local-gu-intra-label-sep)
  (list))

(define (gu-intra-label-sep)
  (list
...
(define (gentext-gu-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-gu-intra-label-sep)))
	 (sep    (assoc name (gu-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-gu-label-title-sep)
  (list))

(define (gu-label-title-sep)
  (list
...
(define (gentext-gu-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-gu-label-title-sep)))
	 (sep    (assoc name (gu-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (gu-label-number-format-list)
  (list
...
(define (gu-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (gu-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-gu$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (gu-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-gu-start-quote% (dingbat "ldquo"))

(define %gentext-gu-end-quote% (dingbat "rdquo"))

(define %gentext-gu-start-nested-quote% (dingbat "lsquo"))

(define %gentext-gu-end-nested-quote% (dingbat "rsquo"))

(define %gentext-gu-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-gu-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-gu-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-gu-page% "")

(define %gentext-gu-and% "&and;")

(define %gentext-gu-listcomma% "&listcomma;")

(define %gentext-gu-lastlistcomma% "&lastlistcomma;")

(define %gentext-gu-bibl-pages% "&Pgs;")

(define %gentext-gu-endnotes% "&Notes;")

(define %gentext-gu-table-endnotes% "&TableNotes;:")

(define %gentext-gu-index-see% "&See;")

(define %gentext-gu-index-seealso% "&SeeAlso;")


(define (gentext-gu-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-gu-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-gu-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-gu-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-gu-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-gu-nav-home home)
