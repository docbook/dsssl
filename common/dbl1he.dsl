...
<!ENTITY % he.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//HE"
         "dbl1he.ent">
%he.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-he">
<style-specification-body>

;; $Id: dbl1he.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (he-author-string #!optional (author (current-node)))
...
(define (he-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-he-start-quote%
						       "%t"
						       %gentext-he-end-quote%))
...
(define (gentext-he-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (he-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (he-auto-xref-indirect-connector before) 
...
;;
(define %generate-he-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (he-element-name)
  (list
...
(define (gentext-he-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (he-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-he-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-he-element-name-space giname)
...
(define (local-he-intra-label-sep)
  (list))

(define (he-intra-label-sep)
  (list
...
(define (gentext-he-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-he-intra-label-sep)))
	 (sep    (assoc name (he-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-he-label-title-sep)
  (list))

(define (he-label-title-sep)
  (list
...
(define (gentext-he-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-he-label-title-sep)))
	 (sep    (assoc name (he-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (he-label-number-format-list)
  (list
...
(define (he-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (he-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-he$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (he-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-he-start-quote% (dingbat "ldquo"))

(define %gentext-he-end-quote% (dingbat "rdquo"))

(define %gentext-he-start-nested-quote% (dingbat "lsquo"))

(define %gentext-he-end-nested-quote% (dingbat "rsquo"))

(define %gentext-he-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-he-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-he-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-he-page% "")

(define %gentext-he-and% "&and;")

(define %gentext-he-listcomma% "&listcomma;")

(define %gentext-he-lastlistcomma% "&lastlistcomma;")

(define %gentext-he-bibl-pages% "&Pgs;")

(define %gentext-he-endnotes% "&Notes;")

(define %gentext-he-table-endnotes% "&TableNotes;:")

(define %gentext-he-index-see% "&See;")

(define %gentext-he-index-seealso% "&SeeAlso;")


(define (gentext-he-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-he-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-he-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-he-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-he-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-he-nav-home home)
