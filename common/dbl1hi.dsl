...
<!ENTITY % hi.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//HI"
         "dbl1hi.ent">
%hi.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-hi">
<style-specification-body>

;; $Id: dbl1hi.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (hi-author-string #!optional (author (current-node)))
...
(define (hi-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-hi-start-quote%
						       "%t"
						       %gentext-hi-end-quote%))
...
(define (gentext-hi-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (hi-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (hi-auto-xref-indirect-connector before) 
...
;;
(define %generate-hi-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (hi-element-name)
  (list
...
(define (gentext-hi-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (hi-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-hi-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-hi-element-name-space giname)
...
(define (local-hi-intra-label-sep)
  (list))

(define (hi-intra-label-sep)
  (list
...
(define (gentext-hi-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-hi-intra-label-sep)))
	 (sep    (assoc name (hi-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-hi-label-title-sep)
  (list))

(define (hi-label-title-sep)
  (list
...
(define (gentext-hi-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-hi-label-title-sep)))
	 (sep    (assoc name (hi-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (hi-label-number-format-list)
  (list
...
(define (hi-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (hi-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-hi$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (hi-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-hi-start-quote% (dingbat "ldquo"))

(define %gentext-hi-end-quote% (dingbat "rdquo"))

(define %gentext-hi-start-nested-quote% (dingbat "lsquo"))

(define %gentext-hi-end-nested-quote% (dingbat "rsquo"))

(define %gentext-hi-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-hi-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-hi-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-hi-page% "")

(define %gentext-hi-and% "&and;")

(define %gentext-hi-listcomma% "&listcomma;")

(define %gentext-hi-lastlistcomma% "&lastlistcomma;")

(define %gentext-hi-bibl-pages% "&Pgs;")

(define %gentext-hi-endnotes% "&Notes;")

(define %gentext-hi-table-endnotes% "&TableNotes;:")

(define %gentext-hi-index-see% "&See;")

(define %gentext-hi-index-seealso% "&SeeAlso;")


(define (gentext-hi-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-hi-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-hi-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-hi-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-hi-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-hi-nav-home home)
