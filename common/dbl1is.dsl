...
<!ENTITY % is.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//IS"
         "dbl1is.ent">
%is.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-is">
<style-specification-body>

;; $Id: dbl1is.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (is-author-string #!optional (author (current-node)))
...
(define (is-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-is-start-quote%
						       "%t"
						       %gentext-is-end-quote%))
...
(define (gentext-is-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (is-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (is-auto-xref-indirect-connector before) 
...
;;
(define %generate-is-toc-in-front% #t)
...
(define (gentext-is-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (is-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-is-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-is-element-name-space giname)
...
(define (local-is-intra-label-sep)
  (list))

(define (is-intra-label-sep)
  (list
...
(define (gentext-is-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-is-intra-label-sep)))
	 (sep    (assoc name (is-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-is-label-title-sep)
  (list))

(define (is-label-title-sep)
  (list
...
(define (gentext-is-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-is-label-title-sep)))
	 (sep    (assoc name (is-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (is-label-number-format-list)
  (list
...
(define (is-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (is-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-is$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (is-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-is-start-quote% (dingbat "ldquo"))

(define %gentext-is-end-quote% (dingbat "rdquo"))

(define %gentext-is-start-nested-quote% (dingbat "lsquo"))

(define %gentext-is-end-nested-quote% (dingbat "rsquo"))

(define %gentext-is-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-is-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-is-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-is-page% "")

(define %gentext-is-and% "&and;")

(define %gentext-is-listcomma% "&listcomma;")

(define %gentext-is-lastlistcomma% "&lastlistcomma;")

(define %gentext-is-bibl-pages% "&Pgs;")

(define %gentext-is-endnotes% "&Notes;")

(define %gentext-is-table-endnotes% "&TableNotes;:")

(define %gentext-is-index-see% "&See;")

(define %gentext-is-index-seealso% "&SeeAlso;")


(define (gentext-is-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-is-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-is-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-is-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-is-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-is-nav-home home)
