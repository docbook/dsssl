...
<!ENTITY % la.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//LA"
         "dbl1la.ent">
%la.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-la">
<style-specification-body>

;; $Id: dbl1la.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (la-author-string #!optional (author (current-node)))
...
(define (la-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-la-start-quote%
						       "%t"
						       %gentext-la-end-quote%))
...
(define (gentext-la-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (la-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (la-auto-xref-indirect-connector before) 
...
;;
(define %generate-la-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (la-element-name)
  (list
...
(define (gentext-la-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (la-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-la-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-la-element-name-space giname)
...
(define (local-la-intra-label-sep)
  (list))

(define (la-intra-label-sep)
  (list
...
(define (gentext-la-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-la-intra-label-sep)))
	 (sep    (assoc name (la-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-la-label-title-sep)
  (list))

(define (la-label-title-sep)
  (list
...
(define (gentext-la-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-la-label-title-sep)))
	 (sep    (assoc name (la-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (la-label-number-format-list)
  (list
...
(define (la-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (la-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-la$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (la-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-la-start-quote% (dingbat "ldquo"))

(define %gentext-la-end-quote% (dingbat "rdquo"))

(define %gentext-la-start-nested-quote% (dingbat "lsquo"))

(define %gentext-la-end-nested-quote% (dingbat "rsquo"))

(define %gentext-la-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-la-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-la-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-la-page% "")

(define %gentext-la-and% "&and;")

(define %gentext-la-listcomma% "&listcomma;")

(define %gentext-la-lastlistcomma% "&lastlistcomma;")

(define %gentext-la-bibl-pages% "&Pgs;")

(define %gentext-la-endnotes% "&Notes;")

(define %gentext-la-table-endnotes% "&TableNotes;:")

(define %gentext-la-index-see% "&See;")

(define %gentext-la-index-seealso% "&SeeAlso;")


(define (gentext-la-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-la-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-la-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-la-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-la-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-la-nav-home home)
