...
<!ENTITY % lt.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//LT"
         "dbl1lt.ent">
%lt.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-lt">
<style-specification-body>

;; $Id: dbl1lt.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (lt-author-string #!optional (author (current-node)))
...
(define (lt-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-lt-start-quote%
						       "%t"
						       %gentext-lt-end-quote%))
...
(define (gentext-lt-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (lt-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (lt-auto-xref-indirect-connector before) 
...
;;
(define %generate-lt-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (lt-element-name)
  (list
...
(define (gentext-lt-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (lt-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-lt-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-lt-element-name-space giname)
...
(define (local-lt-intra-label-sep)
  (list))

(define (lt-intra-label-sep)
  (list
...
(define (gentext-lt-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-lt-intra-label-sep)))
	 (sep    (assoc name (lt-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-lt-label-title-sep)
  (list))

(define (lt-label-title-sep)
  (list
...
(define (gentext-lt-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-lt-label-title-sep)))
	 (sep    (assoc name (lt-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (lt-label-number-format-list)
  (list
...
(define (lt-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (lt-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-lt$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (lt-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-lt-start-quote% (dingbat "ldquo"))

(define %gentext-lt-end-quote% (dingbat "rdquo"))

(define %gentext-lt-start-nested-quote% (dingbat "lsquo"))

(define %gentext-lt-end-nested-quote% (dingbat "rsquo"))

(define %gentext-lt-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-lt-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-lt-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-lt-page% "")

(define %gentext-lt-and% "&and;")

(define %gentext-lt-listcomma% "&listcomma;")

(define %gentext-lt-lastlistcomma% "&lastlistcomma;")

(define %gentext-lt-bibl-pages% "&Pgs;")

(define %gentext-lt-endnotes% "&Notes;")

(define %gentext-lt-table-endnotes% "&TableNotes;:")

(define %gentext-lt-index-see% "&See;")

(define %gentext-lt-index-seealso% "&SeeAlso;")


(define (gentext-lt-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-lt-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-lt-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-lt-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-lt-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-lt-nav-home home)
