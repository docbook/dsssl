...
<!ENTITY % mr.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//MR"
         "dbl1mr.ent">
%mr.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-mr">
<style-specification-body>

;; $Id: dbl1mr.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (mr-author-string #!optional (author (current-node)))
...
(define (mr-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-mr-start-quote%
						       "%t"
						       %gentext-mr-end-quote%))
...
(define (gentext-mr-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (mr-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (mr-auto-xref-indirect-connector before) 
...
;;
(define %generate-mr-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (mr-element-name)
  (list
...
(define (gentext-mr-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (mr-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-mr-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-mr-element-name-space giname)
...
(define (local-mr-intra-label-sep)
  (list))

(define (mr-intra-label-sep)
  (list
...
(define (gentext-mr-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-mr-intra-label-sep)))
	 (sep    (assoc name (mr-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-mr-label-title-sep)
  (list))

(define (mr-label-title-sep)
  (list
...
(define (gentext-mr-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-mr-label-title-sep)))
	 (sep    (assoc name (mr-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (mr-label-number-format-list)
  (list
...
(define (mr-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (mr-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-mr$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (mr-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-mr-start-quote% (dingbat "ldquo"))

(define %gentext-mr-end-quote% (dingbat "rdquo"))

(define %gentext-mr-start-nested-quote% (dingbat "lsquo"))

(define %gentext-mr-end-nested-quote% (dingbat "rsquo"))

(define %gentext-mr-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-mr-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-mr-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-mr-page% "")

(define %gentext-mr-and% "&and;")

(define %gentext-mr-listcomma% "&listcomma;")

(define %gentext-mr-lastlistcomma% "&lastlistcomma;")

(define %gentext-mr-bibl-pages% "&Pgs;")

(define %gentext-mr-endnotes% "&Notes;")

(define %gentext-mr-table-endnotes% "&TableNotes;:")

(define %gentext-mr-index-see% "&See;")

(define %gentext-mr-index-seealso% "&SeeAlso;")


(define (gentext-mr-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-mr-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-mr-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-mr-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-mr-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-mr-nav-home home)
