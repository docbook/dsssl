...
<!ENTITY % nds.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//NDS"
         "dbl1nds.ent">
%nds.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-nds">
<style-specification-body>

;; $Id: dbl1nds.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (nds-author-string #!optional (author (current-node)))
...
(define (nds-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-nds-start-quote%
						       "%t"
						       %gentext-nds-end-quote%))
...
(define (gentext-nds-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (nds-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (nds-auto-xref-indirect-connector before) 
...
;;
(define %generate-nds-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (nds-element-name)
  (list
...
(define (gentext-nds-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (nds-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-nds-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-nds-element-name-space giname)
...
(define (local-nds-intra-label-sep)
  (list))

(define (nds-intra-label-sep)
  (list
...
(define (gentext-nds-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-nds-intra-label-sep)))
	 (sep    (assoc name (nds-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-nds-label-title-sep)
  (list))

(define (nds-label-title-sep)
  (list
...
(define (gentext-nds-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-nds-label-title-sep)))
	 (sep    (assoc name (nds-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (nds-label-number-format-list)
  (list
...
(define (nds-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (nds-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))

(define (nds-lot-title)
  (list
...
(define ($lot-title-nds$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (nds-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-nds-start-quote% (dingbat "ldquo"))

(define %gentext-nds-end-quote% (dingbat "rdquo"))

(define %gentext-nds-start-nested-quote% (dingbat "lsquo"))

(define %gentext-nds-end-nested-quote% (dingbat "rsquo"))

(define %gentext-nds-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-nds-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-nds-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-nds-page% "")

(define %gentext-nds-and% "&and;")

(define %gentext-nds-listcomma% "&listcomma;")

(define %gentext-nds-lastlistcomma% "&lastlistcomma;")

(define %gentext-nds-bibl-pages% "&Pgs;")

(define %gentext-nds-endnotes% "&Notes;")

(define %gentext-nds-table-endnotes% "&TableNotes;:")

(define %gentext-nds-index-see% "&See;")

(define %gentext-nds-index-seealso% "&SeeAlso;")


(define (gentext-nds-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-nds-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-nds-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-nds-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-nds-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-nds-nav-home home)
