...
<!ENTITY % or.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//OR"
         "dbl1or.ent">
%or.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-or">
<style-specification-body>

;; $Id: dbl1or.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (or-author-string #!optional (author (current-node)))
...
(define (or-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-or-start-quote%
						       "%t"
						       %gentext-or-end-quote%))
...
(define (gentext-or-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (or-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (or-auto-xref-indirect-connector before) 
...
;;
(define %generate-or-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (or-element-name)
  (list
...
(define (gentext-or-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (or-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-or-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-or-element-name-space giname)
...
(define (local-or-intra-label-sep)
  (list))

(define (or-intra-label-sep)
  (list
...
(define (gentext-or-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-or-intra-label-sep)))
	 (sep    (assoc name (or-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-or-label-title-sep)
  (list))

(define (or-label-title-sep)
  (list
...
(define (gentext-or-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-or-label-title-sep)))
	 (sep    (assoc name (or-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (or-label-number-format-list)
  (list
...
(define (or-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (or-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-or$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (or-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-or-start-quote% (dingbat "ldquo"))

(define %gentext-or-end-quote% (dingbat "rdquo"))

(define %gentext-or-start-nested-quote% (dingbat "lsquo"))

(define %gentext-or-end-nested-quote% (dingbat "rsquo"))

(define %gentext-or-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-or-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-or-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-or-page% "")

(define %gentext-or-and% "&and;")

(define %gentext-or-listcomma% "&listcomma;")

(define %gentext-or-lastlistcomma% "&lastlistcomma;")

(define %gentext-or-bibl-pages% "&Pgs;")

(define %gentext-or-endnotes% "&Notes;")

(define %gentext-or-table-endnotes% "&TableNotes;:")

(define %gentext-or-index-see% "&See;")

(define %gentext-or-index-seealso% "&SeeAlso;")


(define (gentext-or-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-or-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-or-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-or-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-or-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-or-nav-home home)
