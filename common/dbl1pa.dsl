...
<!ENTITY % pa.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//PA"
         "dbl1pa.ent">
%pa.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-pa">
<style-specification-body>

;; $Id: dbl1pa.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (pa-author-string #!optional (author (current-node)))
...
(define (pa-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-pa-start-quote%
						       "%t"
						       %gentext-pa-end-quote%))
...
(define (gentext-pa-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (pa-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (pa-auto-xref-indirect-connector before) 
...
;;
(define %generate-pa-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (pa-element-name)
  (list
...
(define (gentext-pa-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (pa-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-pa-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-pa-element-name-space giname)
...
(define (local-pa-intra-label-sep)
  (list))

(define (pa-intra-label-sep)
  (list
...
(define (gentext-pa-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-pa-intra-label-sep)))
	 (sep    (assoc name (pa-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-pa-label-title-sep)
  (list))

(define (pa-label-title-sep)
  (list
...
(define (gentext-pa-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-pa-label-title-sep)))
	 (sep    (assoc name (pa-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (pa-label-number-format-list)
  (list
...
(define (pa-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (pa-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-pa$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (pa-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-pa-start-quote% (dingbat "ldquo"))

(define %gentext-pa-end-quote% (dingbat "rdquo"))

(define %gentext-pa-start-nested-quote% (dingbat "lsquo"))

(define %gentext-pa-end-nested-quote% (dingbat "rsquo"))

(define %gentext-pa-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-pa-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-pa-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-pa-page% "")

(define %gentext-pa-and% "&and;")

(define %gentext-pa-listcomma% "&listcomma;")

(define %gentext-pa-lastlistcomma% "&lastlistcomma;")

(define %gentext-pa-bibl-pages% "&Pgs;")

(define %gentext-pa-endnotes% "&Notes;")

(define %gentext-pa-table-endnotes% "&TableNotes;:")

(define %gentext-pa-index-see% "&See;")

(define %gentext-pa-index-seealso% "&SeeAlso;")


(define (gentext-pa-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-pa-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-pa-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-pa-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-pa-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-pa-nav-home home)
