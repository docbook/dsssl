...
<!ENTITY % srLatn.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//SRLATN"
         "dbl1srLatn.ent">
%srLatn.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-srLatn">
<style-specification-body>

;; $Id: dbl1srLatn.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (srLatn-author-string #!optional (author (current-node)))
...
(define (srLatn-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-srLatn-start-quote%
						       "%t"
						       %gentext-srLatn-end-quote%))
...
(define (gentext-srLatn-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (srLatn-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (srLatn-auto-xref-indirect-connector before) 
...
;;
(define %generate-srLatn-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (srLatn-element-name)
  (list
...
(define (gentext-srLatn-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (srLatn-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-srLatn-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-srLatn-element-name-space giname)
...
(define (local-srLatn-intra-label-sep)
  (list))

(define (srLatn-intra-label-sep)
  (list
...
(define (gentext-srLatn-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-srLatn-intra-label-sep)))
	 (sep    (assoc name (srLatn-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-srLatn-label-title-sep)
  (list))

(define (srLatn-label-title-sep)
  (list
...
(define (gentext-srLatn-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-srLatn-label-title-sep)))
	 (sep    (assoc name (srLatn-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (srLatn-label-number-format-list)
  (list
...
(define (srLatn-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (srLatn-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))

(define (srLatn-lot-title)
  (list
...
(define ($lot-title-srLatn$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (srLatn-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-srLatn-start-quote% (dingbat "ldquo"))

(define %gentext-srLatn-end-quote% (dingbat "rdquo"))

(define %gentext-srLatn-start-nested-quote% (dingbat "lsquo"))

(define %gentext-srLatn-end-nested-quote% (dingbat "rsquo"))

(define %gentext-srLatn-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-srLatn-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-srLatn-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-srLatn-page% "")

(define %gentext-srLatn-and% "&and;")

(define %gentext-srLatn-listcomma% "&listcomma;")

(define %gentext-srLatn-lastlistcomma% "&lastlistcomma;")

(define %gentext-srLatn-bibl-pages% "&Pgs;")

(define %gentext-srLatn-endnotes% "&Notes;")

(define %gentext-srLatn-table-endnotes% "&TableNotes;:")

(define %gentext-srLatn-index-see% "&See;")

(define %gentext-srLatn-index-seealso% "&SeeAlso;")


(define (gentext-srLatn-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-srLatn-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-srLatn-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-srLatn-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-srLatn-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-srLatn-nav-home home)
