...
<!ENTITY % ta.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//TA"
         "dbl1ta.ent">
%ta.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-ta">
<style-specification-body>

;; $Id: dbl1ta.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (ta-author-string #!optional (author (current-node)))
...
(define (ta-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-ta-start-quote%
						       "%t"
						       %gentext-ta-end-quote%))
...
(define (gentext-ta-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (ta-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (ta-auto-xref-indirect-connector before) 
...
;;
(define %generate-ta-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (ta-element-name)
  (list
...
(define (gentext-ta-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (ta-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-ta-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-ta-element-name-space giname)
...
(define (local-ta-intra-label-sep)
  (list))

(define (ta-intra-label-sep)
  (list
...
(define (gentext-ta-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-ta-intra-label-sep)))
	 (sep    (assoc name (ta-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-ta-label-title-sep)
  (list))

(define (ta-label-title-sep)
  (list
...
(define (gentext-ta-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-ta-label-title-sep)))
	 (sep    (assoc name (ta-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (ta-label-number-format-list)
  (list
...
(define (ta-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (ta-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-ta$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (ta-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-ta-start-quote% (dingbat "ldquo"))

(define %gentext-ta-end-quote% (dingbat "rdquo"))

(define %gentext-ta-start-nested-quote% (dingbat "lsquo"))

(define %gentext-ta-end-nested-quote% (dingbat "rsquo"))

(define %gentext-ta-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-ta-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-ta-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-ta-page% "")

(define %gentext-ta-and% "&and;")

(define %gentext-ta-listcomma% "&listcomma;")

(define %gentext-ta-lastlistcomma% "&lastlistcomma;")

(define %gentext-ta-bibl-pages% "&Pgs;")

(define %gentext-ta-endnotes% "&Notes;")

(define %gentext-ta-table-endnotes% "&TableNotes;:")

(define %gentext-ta-index-see% "&See;")

(define %gentext-ta-index-seealso% "&SeeAlso;")


(define (gentext-ta-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-ta-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-ta-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-ta-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-ta-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-ta-nav-home home)
