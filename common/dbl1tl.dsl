...
<!ENTITY % tl.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//TL"
         "dbl1tl.ent">
%tl.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-tl">
<style-specification-body>

;; $Id: dbl1tl.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (tl-author-string #!optional (author (current-node)))
...
(define (tl-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-tl-start-quote%
						       "%t"
						       %gentext-tl-end-quote%))
...
(define (gentext-tl-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (tl-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (tl-auto-xref-indirect-connector before) 
...
;;
(define %generate-tl-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (tl-element-name)
  (list
...
(define (gentext-tl-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (tl-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-tl-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-tl-element-name-space giname)
...
(define (local-tl-intra-label-sep)
  (list))

(define (tl-intra-label-sep)
  (list
...
(define (gentext-tl-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-tl-intra-label-sep)))
	 (sep    (assoc name (tl-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-tl-label-title-sep)
  (list))

(define (tl-label-title-sep)
  (list
...
(define (gentext-tl-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-tl-label-title-sep)))
	 (sep    (assoc name (tl-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (tl-label-number-format-list)
  (list
...
(define (tl-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (tl-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-tl$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (tl-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-tl-start-quote% (dingbat "ldquo"))

(define %gentext-tl-end-quote% (dingbat "rdquo"))

(define %gentext-tl-start-nested-quote% (dingbat "lsquo"))

(define %gentext-tl-end-nested-quote% (dingbat "rsquo"))

(define %gentext-tl-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-tl-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-tl-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-tl-page% "")

(define %gentext-tl-and% "&and;")

(define %gentext-tl-listcomma% "&listcomma;")

(define %gentext-tl-lastlistcomma% "&lastlistcomma;")

(define %gentext-tl-bibl-pages% "&Pgs;")

(define %gentext-tl-endnotes% "&Notes;")

(define %gentext-tl-table-endnotes% "&TableNotes;:")

(define %gentext-tl-index-see% "&See;")

(define %gentext-tl-index-seealso% "&SeeAlso;")


(define (gentext-tl-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-tl-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-tl-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-tl-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-tl-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-tl-nav-home home)
