...
<!ENTITY % am.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//AM"
         "dbl1am.ent">
%am.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-am">
<style-specification-body>

;; $Id: dbl1am.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (am-author-string #!optional (author (current-node)))
...
(define (am-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-am-start-quote%
						       "%t"
						       %gentext-am-end-quote%))
...
(define (gentext-am-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (am-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (am-auto-xref-indirect-connector before) 
...
;;
(define %generate-am-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (am-element-name)
  (list
...
(define (gentext-am-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (am-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-am-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-am-element-name-space giname)
...
(define (local-am-intra-label-sep)
  (list))

(define (am-intra-label-sep)
  (list
...
(define (gentext-am-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-am-intra-label-sep)))
	 (sep    (assoc name (am-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-am-label-title-sep)
  (list))

(define (am-label-title-sep)
  (list
...
(define (gentext-am-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-am-label-title-sep)))
	 (sep    (assoc name (am-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (am-label-number-format-list)
  (list
...
(define (am-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (am-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-am$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (am-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-am-start-quote% (dingbat "ldquo"))

(define %gentext-am-end-quote% (dingbat "rdquo"))

(define %gentext-am-start-nested-quote% (dingbat "lsquo"))

(define %gentext-am-end-nested-quote% (dingbat "rsquo"))

(define %gentext-am-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-am-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-am-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-am-page% "")

(define %gentext-am-and% "&and;")

(define %gentext-am-listcomma% "&listcomma;")

(define %gentext-am-lastlistcomma% "&lastlistcomma;")

(define %gentext-am-bibl-pages% "&Pgs;")

(define %gentext-am-endnotes% "&Notes;")

(define %gentext-am-table-endnotes% "&TableNotes;:")

(define %gentext-am-index-see% "&See;")

(define %gentext-am-index-seealso% "&SeeAlso;")


(define (gentext-am-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-am-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-am-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-am-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-am-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-am-nav-home home)
