...
<!ENTITY % ast.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//AST"
         "dbl1ast.ent">
%ast.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-ast">
<style-specification-body>

;; $Id: dbl1ast.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (ast-author-string #!optional (author (current-node)))
...
(define (ast-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-ast-start-quote%
						       "%t"
						       %gentext-ast-end-quote%))
...
(define (gentext-ast-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (ast-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (ast-auto-xref-indirect-connector before) 
...
;;
(define %generate-ast-toc-in-front% #t)
...
(define (gentext-ast-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (ast-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-ast-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-ast-element-name-space giname)
...
(define (local-ast-intra-label-sep)
  (list))

(define (ast-intra-label-sep)
  (list
...
(define (gentext-ast-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-ast-intra-label-sep)))
	 (sep    (assoc name (ast-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-ast-label-title-sep)
  (list))

(define (ast-label-title-sep)
  (list
...
(define (gentext-ast-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-ast-label-title-sep)))
	 (sep    (assoc name (ast-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (ast-label-number-format-list)
  (list
...
(define (ast-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (ast-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))

(define (ast-lot-title)
  (list
...
(define ($lot-title-ast$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (ast-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-ast-start-quote% (dingbat "ldquo"))

(define %gentext-ast-end-quote% (dingbat "rdquo"))

(define %gentext-ast-start-nested-quote% (dingbat "lsquo"))

(define %gentext-ast-end-nested-quote% (dingbat "rsquo"))

(define %gentext-ast-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-ast-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-ast-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-ast-page% "")

(define %gentext-ast-and% "&and;")

(define %gentext-ast-listcomma% "&listcomma;")

(define %gentext-ast-lastlistcomma% "&lastlistcomma;")

(define %gentext-ast-bibl-pages% "&Pgs;")

(define %gentext-ast-endnotes% "&Notes;")

(define %gentext-ast-table-endnotes% "&TableNotes;:")

(define %gentext-ast-index-see% "&See;")

(define %gentext-ast-index-seealso% "&SeeAlso;")


(define (gentext-ast-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-ast-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-ast-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-ast-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-ast-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-ast-nav-home home)
