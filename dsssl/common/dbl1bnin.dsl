...
<!ENTITY % bnin.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//BNIN"
         "dbl1bnin.ent">
%bnin.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-bnin">
<style-specification-body>

;; $Id: dbl1bnin.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (bnin-author-string #!optional (author (current-node)))
...
(define (bnin-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-bnin-start-quote%
						       "%t"
						       %gentext-bnin-end-quote%))
...
(define (gentext-bnin-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (bnin-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (bnin-auto-xref-indirect-connector before) 
...
;;
(define %generate-bnin-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (bnin-element-name)
  (list
...
(define (gentext-bnin-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (bnin-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-bnin-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-bnin-element-name-space giname)
...
(define (local-bnin-intra-label-sep)
  (list))

(define (bnin-intra-label-sep)
  (list
...
(define (gentext-bnin-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-bnin-intra-label-sep)))
	 (sep    (assoc name (bnin-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-bnin-label-title-sep)
  (list))

(define (bnin-label-title-sep)
  (list
...
(define (gentext-bnin-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-bnin-label-title-sep)))
	 (sep    (assoc name (bnin-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (bnin-label-number-format-list)
  (list
...
(define (bnin-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (bnin-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))

(define (bnin-lot-title)
  (list
...
(define ($lot-title-bnin$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (bnin-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-bnin-start-quote% (dingbat "ldquo"))

(define %gentext-bnin-end-quote% (dingbat "rdquo"))

(define %gentext-bnin-start-nested-quote% (dingbat "lsquo"))

(define %gentext-bnin-end-nested-quote% (dingbat "rsquo"))

(define %gentext-bnin-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-bnin-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-bnin-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-bnin-page% "")

(define %gentext-bnin-and% "&and;")

(define %gentext-bnin-listcomma% "&listcomma;")

(define %gentext-bnin-lastlistcomma% "&lastlistcomma;")

(define %gentext-bnin-bibl-pages% "&Pgs;")

(define %gentext-bnin-endnotes% "&Notes;")

(define %gentext-bnin-table-endnotes% "&TableNotes;:")

(define %gentext-bnin-index-see% "&See;")

(define %gentext-bnin-index-seealso% "&SeeAlso;")


(define (gentext-bnin-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-bnin-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-bnin-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-bnin-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-bnin-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-bnin-nav-home home)
