...
<!ENTITY % eo.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//EO"
         "dbl1eo.ent">
%eo.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-eo">
<style-specification-body>

;; $Id: dbl1eo.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (eo-author-string #!optional (author (current-node)))
...
(define (eo-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-eo-start-quote%
						       "%t"
						       %gentext-eo-end-quote%))
...
(define (gentext-eo-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (eo-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (eo-auto-xref-indirect-connector before) 
...
;;
(define %generate-eo-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (eo-element-name)
  (list
...
(define (gentext-eo-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (eo-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-eo-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-eo-element-name-space giname)
...
(define (local-eo-intra-label-sep)
  (list))

(define (eo-intra-label-sep)
  (list
...
(define (gentext-eo-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-eo-intra-label-sep)))
	 (sep    (assoc name (eo-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-eo-label-title-sep)
  (list))

(define (eo-label-title-sep)
  (list
...
(define (gentext-eo-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-eo-label-title-sep)))
	 (sep    (assoc name (eo-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (eo-label-number-format-list)
  (list
...
(define (eo-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (eo-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-eo$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (eo-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-eo-start-quote% (dingbat "ldquo"))

(define %gentext-eo-end-quote% (dingbat "rdquo"))

(define %gentext-eo-start-nested-quote% (dingbat "lsquo"))

(define %gentext-eo-end-nested-quote% (dingbat "rsquo"))

(define %gentext-eo-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-eo-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-eo-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-eo-page% "")

(define %gentext-eo-and% "&and;")

(define %gentext-eo-listcomma% "&listcomma;")

(define %gentext-eo-lastlistcomma% "&lastlistcomma;")

(define %gentext-eo-bibl-pages% "&Pgs;")

(define %gentext-eo-endnotes% "&Notes;")

(define %gentext-eo-table-endnotes% "&TableNotes;:")

(define %gentext-eo-index-see% "&See;")

(define %gentext-eo-index-seealso% "&SeeAlso;")


(define (gentext-eo-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-eo-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-eo-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-eo-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-eo-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-eo-nav-home home)
