...
<!ENTITY % gl.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//GL"
         "dbl1gl.ent">
%gl.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-gl">
<style-specification-body>

;; $Id: dbl1gl.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (gl-author-string #!optional (author (current-node)))
...
(define (gl-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-gl-start-quote%
						       "%t"
						       %gentext-gl-end-quote%))
...
(define (gentext-gl-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (gl-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (gl-auto-xref-indirect-connector before) 
...
;;
(define %generate-gl-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (gl-element-name)
  (list
...
(define (gentext-gl-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (gl-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-gl-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-gl-element-name-space giname)
...
(define (local-gl-intra-label-sep)
  (list))

(define (gl-intra-label-sep)
  (list
...
(define (gentext-gl-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-gl-intra-label-sep)))
	 (sep    (assoc name (gl-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-gl-label-title-sep)
  (list))

(define (gl-label-title-sep)
  (list
...
(define (gentext-gl-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-gl-label-title-sep)))
	 (sep    (assoc name (gl-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (gl-label-number-format-list)
  (list
...
(define (gl-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (gl-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-gl$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (gl-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-gl-start-quote% (dingbat "ldquo"))

(define %gentext-gl-end-quote% (dingbat "rdquo"))

(define %gentext-gl-start-nested-quote% (dingbat "lsquo"))

(define %gentext-gl-end-nested-quote% (dingbat "rsquo"))

(define %gentext-gl-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-gl-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-gl-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-gl-page% "")

(define %gentext-gl-and% "&and;")

(define %gentext-gl-listcomma% "&listcomma;")

(define %gentext-gl-lastlistcomma% "&lastlistcomma;")

(define %gentext-gl-bibl-pages% "&Pgs;")

(define %gentext-gl-endnotes% "&Notes;")

(define %gentext-gl-table-endnotes% "&TableNotes;:")

(define %gentext-gl-index-see% "&See;")

(define %gentext-gl-index-seealso% "&SeeAlso;")


(define (gentext-gl-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-gl-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-gl-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-gl-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-gl-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-gl-nav-home home)
