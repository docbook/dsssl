...
<!ENTITY % hr.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//HR"
         "dbl1hr.ent">
%hr.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-hr">
<style-specification-body>

;; $Id: dbl1hr.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (hr-author-string #!optional (author (current-node)))
...
(define (hr-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-hr-start-quote%
						       "%t"
						       %gentext-hr-end-quote%))
...
(define (gentext-hr-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (hr-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (hr-auto-xref-indirect-connector before) 
...
;;
(define %generate-hr-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (hr-element-name)
  (list
...
(define (gentext-hr-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (hr-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-hr-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-hr-element-name-space giname)
...
(define (local-hr-intra-label-sep)
  (list))

(define (hr-intra-label-sep)
  (list
...
(define (gentext-hr-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-hr-intra-label-sep)))
	 (sep    (assoc name (hr-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-hr-label-title-sep)
  (list))

(define (hr-label-title-sep)
  (list
...
(define (gentext-hr-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-hr-label-title-sep)))
	 (sep    (assoc name (hr-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (hr-label-number-format-list)
  (list
...
(define (hr-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (hr-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-hr$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (hr-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-hr-start-quote% (dingbat "ldquo"))

(define %gentext-hr-end-quote% (dingbat "rdquo"))

(define %gentext-hr-start-nested-quote% (dingbat "lsquo"))

(define %gentext-hr-end-nested-quote% (dingbat "rsquo"))

(define %gentext-hr-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-hr-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-hr-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-hr-page% "")

(define %gentext-hr-and% "&and;")

(define %gentext-hr-listcomma% "&listcomma;")

(define %gentext-hr-lastlistcomma% "&lastlistcomma;")

(define %gentext-hr-bibl-pages% "&Pgs;")

(define %gentext-hr-endnotes% "&Notes;")

(define %gentext-hr-table-endnotes% "&TableNotes;:")

(define %gentext-hr-index-see% "&See;")

(define %gentext-hr-index-seealso% "&SeeAlso;")


(define (gentext-hr-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-hr-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-hr-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-hr-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-hr-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-hr-nav-home home)
