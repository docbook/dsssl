...
<!ENTITY % ka.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//KA"
         "dbl1ka.ent">
%ka.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-ka">
<style-specification-body>

;; $Id: dbl1ka.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (ka-author-string #!optional (author (current-node)))
...
(define (ka-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-ka-start-quote%
						       "%t"
						       %gentext-ka-end-quote%))
...
(define (gentext-ka-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (ka-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (ka-auto-xref-indirect-connector before) 
...
;;
(define %generate-ka-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (ka-element-name)
  (list
...
(define (gentext-ka-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (ka-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-ka-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-ka-element-name-space giname)
...
(define (local-ka-intra-label-sep)
  (list))

(define (ka-intra-label-sep)
  (list
...
(define (gentext-ka-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-ka-intra-label-sep)))
	 (sep    (assoc name (ka-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-ka-label-title-sep)
  (list))

(define (ka-label-title-sep)
  (list
...
(define (gentext-ka-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-ka-label-title-sep)))
	 (sep    (assoc name (ka-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (ka-label-number-format-list)
  (list
...
(define (ka-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (ka-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-ka$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (ka-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-ka-start-quote% (dingbat "ldquo"))

(define %gentext-ka-end-quote% (dingbat "rdquo"))

(define %gentext-ka-start-nested-quote% (dingbat "lsquo"))

(define %gentext-ka-end-nested-quote% (dingbat "rsquo"))

(define %gentext-ka-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-ka-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-ka-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-ka-page% "")

(define %gentext-ka-and% "&and;")

(define %gentext-ka-listcomma% "&listcomma;")

(define %gentext-ka-lastlistcomma% "&lastlistcomma;")

(define %gentext-ka-bibl-pages% "&Pgs;")

(define %gentext-ka-endnotes% "&Notes;")

(define %gentext-ka-table-endnotes% "&TableNotes;:")

(define %gentext-ka-index-see% "&See;")

(define %gentext-ka-index-seealso% "&SeeAlso;")


(define (gentext-ka-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-ka-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-ka-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-ka-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-ka-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-ka-nav-home home)
