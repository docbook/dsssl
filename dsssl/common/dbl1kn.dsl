...
<!ENTITY % kn.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//KN"
         "dbl1kn.ent">
%kn.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-kn">
<style-specification-body>

;; $Id: dbl1kn.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (kn-author-string #!optional (author (current-node)))
...
(define (kn-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-kn-start-quote%
						       "%t"
						       %gentext-kn-end-quote%))
...
(define (gentext-kn-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (kn-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (kn-auto-xref-indirect-connector before) 
...
;;
(define %generate-kn-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (kn-element-name)
  (list
...
(define (gentext-kn-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (kn-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-kn-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-kn-element-name-space giname)
...
(define (local-kn-intra-label-sep)
  (list))

(define (kn-intra-label-sep)
  (list
...
(define (gentext-kn-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-kn-intra-label-sep)))
	 (sep    (assoc name (kn-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-kn-label-title-sep)
  (list))

(define (kn-label-title-sep)
  (list
...
(define (gentext-kn-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-kn-label-title-sep)))
	 (sep    (assoc name (kn-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (kn-label-number-format-list)
  (list
...
(define (kn-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (kn-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-kn$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (kn-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-kn-start-quote% (dingbat "ldquo"))

(define %gentext-kn-end-quote% (dingbat "rdquo"))

(define %gentext-kn-start-nested-quote% (dingbat "lsquo"))

(define %gentext-kn-end-nested-quote% (dingbat "rsquo"))

(define %gentext-kn-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-kn-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-kn-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-kn-page% "")

(define %gentext-kn-and% "&and;")

(define %gentext-kn-listcomma% "&listcomma;")

(define %gentext-kn-lastlistcomma% "&lastlistcomma;")

(define %gentext-kn-bibl-pages% "&Pgs;")

(define %gentext-kn-endnotes% "&Notes;")

(define %gentext-kn-table-endnotes% "&TableNotes;:")

(define %gentext-kn-index-see% "&See;")

(define %gentext-kn-index-seealso% "&SeeAlso;")


(define (gentext-kn-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-kn-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-kn-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-kn-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-kn-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-kn-nav-home home)
