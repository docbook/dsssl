...
<!ENTITY % lv.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//LV"
         "dbl1lv.ent">
%lv.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-lv">
<style-specification-body>

;; $Id: dbl1lv.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (lv-author-string #!optional (author (current-node)))
...
(define (lv-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-lv-start-quote%
						       "%t"
						       %gentext-lv-end-quote%))
...
(define (gentext-lv-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (lv-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (lv-auto-xref-indirect-connector before) 
...
;;
(define %generate-lv-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (lv-element-name)
  (list
...
(define (gentext-lv-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (lv-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-lv-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-lv-element-name-space giname)
...
(define (local-lv-intra-label-sep)
  (list))

(define (lv-intra-label-sep)
  (list
...
(define (gentext-lv-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-lv-intra-label-sep)))
	 (sep    (assoc name (lv-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-lv-label-title-sep)
  (list))

(define (lv-label-title-sep)
  (list
...
(define (gentext-lv-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-lv-label-title-sep)))
	 (sep    (assoc name (lv-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (lv-label-number-format-list)
  (list
...
(define (lv-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (lv-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-lv$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (lv-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-lv-start-quote% (dingbat "ldquo"))

(define %gentext-lv-end-quote% (dingbat "rdquo"))

(define %gentext-lv-start-nested-quote% (dingbat "lsquo"))

(define %gentext-lv-end-nested-quote% (dingbat "rsquo"))

(define %gentext-lv-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-lv-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-lv-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-lv-page% "")

(define %gentext-lv-and% "&and;")

(define %gentext-lv-listcomma% "&listcomma;")

(define %gentext-lv-lastlistcomma% "&lastlistcomma;")

(define %gentext-lv-bibl-pages% "&Pgs;")

(define %gentext-lv-endnotes% "&Notes;")

(define %gentext-lv-table-endnotes% "&TableNotes;:")

(define %gentext-lv-index-see% "&See;")

(define %gentext-lv-index-seealso% "&SeeAlso;")


(define (gentext-lv-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-lv-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-lv-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-lv-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-lv-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-lv-nav-home home)
