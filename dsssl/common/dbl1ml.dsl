...
<!ENTITY % ml.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//ML"
         "dbl1ml.ent">
%ml.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-ml">
<style-specification-body>

;; $Id: dbl1ml.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (ml-author-string #!optional (author (current-node)))
...
(define (ml-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-ml-start-quote%
						       "%t"
						       %gentext-ml-end-quote%))
...
(define (gentext-ml-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (ml-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (ml-auto-xref-indirect-connector before) 
...
;;
(define %generate-ml-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (ml-element-name)
  (list
...
(define (gentext-ml-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (ml-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-ml-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-ml-element-name-space giname)
...
(define (local-ml-intra-label-sep)
  (list))

(define (ml-intra-label-sep)
  (list
...
(define (gentext-ml-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-ml-intra-label-sep)))
	 (sep    (assoc name (ml-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-ml-label-title-sep)
  (list))

(define (ml-label-title-sep)
  (list
...
(define (gentext-ml-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-ml-label-title-sep)))
	 (sep    (assoc name (ml-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (ml-label-number-format-list)
  (list
...
(define (ml-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (ml-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-ml$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (ml-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-ml-start-quote% (dingbat "ldquo"))

(define %gentext-ml-end-quote% (dingbat "rdquo"))

(define %gentext-ml-start-nested-quote% (dingbat "lsquo"))

(define %gentext-ml-end-nested-quote% (dingbat "rsquo"))

(define %gentext-ml-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-ml-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-ml-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-ml-page% "")

(define %gentext-ml-and% "&and;")

(define %gentext-ml-listcomma% "&listcomma;")

(define %gentext-ml-lastlistcomma% "&lastlistcomma;")

(define %gentext-ml-bibl-pages% "&Pgs;")

(define %gentext-ml-endnotes% "&Notes;")

(define %gentext-ml-table-endnotes% "&TableNotes;:")

(define %gentext-ml-index-see% "&See;")

(define %gentext-ml-index-seealso% "&SeeAlso;")


(define (gentext-ml-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-ml-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-ml-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-ml-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-ml-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-ml-nav-home home)
