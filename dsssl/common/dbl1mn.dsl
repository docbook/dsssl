...
<!ENTITY % mn.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//MN"
         "dbl1mn.ent">
%mn.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-mn">
<style-specification-body>

;; $Id: dbl1mn.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (mn-author-string #!optional (author (current-node)))
...
(define (mn-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-mn-start-quote%
						       "%t"
						       %gentext-mn-end-quote%))
...
(define (gentext-mn-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (mn-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (mn-auto-xref-indirect-connector before) 
...
;;
(define %generate-mn-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (mn-element-name)
  (list
...
(define (gentext-mn-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (mn-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-mn-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-mn-element-name-space giname)
...
(define (local-mn-intra-label-sep)
  (list))

(define (mn-intra-label-sep)
  (list
...
(define (gentext-mn-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-mn-intra-label-sep)))
	 (sep    (assoc name (mn-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (gentext-mn-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-mn-label-title-sep)))
	 (sep    (assoc name (mn-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (mn-label-number-format-list)
  (list
...
(define (mn-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (mn-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-mn$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (mn-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-mn-start-quote% (dingbat "ldquo"))

(define %gentext-mn-end-quote% (dingbat "rdquo"))

(define %gentext-mn-start-nested-quote% (dingbat "lsquo"))

(define %gentext-mn-end-nested-quote% (dingbat "rsquo"))

(define %gentext-mn-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-mn-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-mn-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-mn-page% "")

(define %gentext-mn-and% "&and;")

(define %gentext-mn-listcomma% "&listcomma;")

(define %gentext-mn-lastlistcomma% "&lastlistcomma;")

(define %gentext-mn-bibl-pages% "&Pgs;")

(define %gentext-mn-endnotes% "&Notes;")

(define %gentext-mn-table-endnotes% "&TableNotes;:")

(define %gentext-mn-index-see% "&See;")

(define %gentext-mn-index-seealso% "&SeeAlso;")


(define (gentext-mn-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-mn-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-mn-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-mn-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-mn-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-mn-nav-home home)
