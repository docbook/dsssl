...
<!ENTITY % te.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//TE"
         "dbl1te.ent">
%te.words;
]>

<style-sheet>
<style-specification id="docbook-l10n-te">
<style-specification-body>

;; $Id: dbl1te.dsl 2010-08-27 04:19:16Z rlandmann $
;;
...
(define (te-author-string #!optional (author (current-node)))
...
(define (te-xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix; %n"
					    "the &appendix; called %t"))
	(list (normalize "article")     (string-append %gentext-te-start-quote%
						       "%t"
						       %gentext-te-end-quote%))
...
(define (gentext-te-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (te-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (te-auto-xref-indirect-connector before) 
...
;;
(define %generate-te-toc-in-front% #t)

;; gentext-element-name returns the generated text that should be 
;; used to make reference to the selected element.
;;

(define (te-element-name)
  (list
...
(define (gentext-te-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (pname  (assoc name (te-element-name))))
    (if pname
	(car (cdr pname))
	(let* ((msg (string-append 
		     "gentext-te-element-name: &unexpectedelementname;: "
		     name))
	       (err (node-list-error msg (current-node))))
	  msg))))
...
;;
(define (gentext-te-element-name-space giname)
...
(define (local-te-intra-label-sep)
  (list))

(define (te-intra-label-sep)
  (list
...
(define (gentext-te-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-te-intra-label-sep)))
	 (sep    (assoc name (te-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-te-label-title-sep)
  (list))

(define (te-label-title-sep)
  (list
...
(define (gentext-te-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-te-label-title-sep)))
	 (sep    (assoc name (te-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (te-label-number-format-list)
  (list
...
(define (te-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (te-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-te$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (te-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-te-start-quote% (dingbat "ldquo"))

(define %gentext-te-end-quote% (dingbat "rdquo"))

(define %gentext-te-start-nested-quote% (dingbat "lsquo"))

(define %gentext-te-end-nested-quote% (dingbat "rsquo"))

(define %gentext-te-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-te-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-te-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-te-page% "")

(define %gentext-te-and% "&and;")

(define %gentext-te-listcomma% "&listcomma;")

(define %gentext-te-lastlistcomma% "&lastlistcomma;")

(define %gentext-te-bibl-pages% "&Pgs;")

(define %gentext-te-endnotes% "&Notes;")

(define %gentext-te-table-endnotes% "&TableNotes;:")

(define %gentext-te-index-see% "&See;")

(define %gentext-te-index-seealso% "&SeeAlso;")


(define (gentext-te-nav-prev prev) 
  (make sequence (literal "&nav-prev;")))

(define (gentext-te-nav-prev-sibling prevsib) 
  (make sequence (literal "&nav-prev-sibling;")))

(define (gentext-te-nav-next-sibling nextsib)
  (make sequence (literal "&nav-next-sibling;")))

(define (gentext-te-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-te-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-te-nav-home home)
